...
namespace JsonDiffPatchDotNet
{
    public class JsonDiffPatch
    {
        ...
        public List<Microsoft.AspNetCore.JsonPatch.Operations.Operation> Diff(JToken left, JToken right)
        {
            if (left == null)
                left = new JValue("");
            if (right == null)
            ...
            {
                return new List<Microsoft.AspNetCore.JsonPatch.Operations.Operation>() { new Microsoft.AspNetCore.JsonPatch.Operations.Operation("replace", left.Path, null, right) };
        ...
        private List<Microsoft.AspNetCore.JsonPatch.Operations.Operation> ObjectDiff(JObject left, JObject right)
        {
            if (left == null)
            ...
            var diffPatch = new List<Microsoft.AspNetCore.JsonPatch.Operations.Operation>();
            ...
            foreach (var lp in left.Properties())
            {
                ...
                if (rp == null)
                {
                    //diffPatch.Add(new Microsoft.AspNetCore.JsonPatch.Operations.Operation("remove", lp.Path, null, null));
                    continue;
                }

                var d = Diff(lp.Value, rp.Value);
                if (d != null)
                {
                    foreach (var operation in d)
                    {
                        diffPatch.Add(operation);
                    }
                    //foreach (var VARIABLE in d)
                    //{
                        
                    //}
                    //diffPatch.Add(new Microsoft.AspNetCore.JsonPatch.Operations.Operation("replace", lp.Path, null, d));
                }
            }
